...
from typing import Any, Optional, Union, Dict, Tuple, Iterator, List, TypeVar, Type
...
def technical_500_response(request: HttpRequest, exc_type: type, exc_value: BaseException, tb: types.TracebackType, status_code: int = 500) -> HttpResponse:
    ...
    """
    reporter: ExceptionReporter = get_exception_reporter_class(request)(request, exc_type, exc_value, tb)
    if request.accepts("text/html"):
        html: str = reporter.get_traceback_html()
        return HttpResponse(html, status=status_code)
    else:
        text: str = reporter.get_traceback_text()
        return HttpResponse(
...
class SafeExceptionReporterFilter:
    ...
    def cleanse_setting(self, key: str, value: T) -> Union[T, CallableSettingWrapper]:
        ...
        """
        is_sensitive: bool
        cleansed: Union[T, CallableSettingWrapper]
        ...
        if is_sensitive:
            ...
            cleansed: dict = {k: self.cleanse_setting(k, v) for k, v in value.items()}
    ...
    def get_safe_settings(self) -> Dict[str, Union[T, CallableSettingWrapper]]:
        ...
        """
        settings_dict: Dict[str, Union[T, CallableSettingWrapper]] = {}
        k: str
    ...
    def get_cleansed_multivaluedict(self, request: HttpRequest, multivaluedict: MultiValueDict) -> MultiValueDict:
        ...
        """
        sensitive_post_parameters: List[str] = getattr(request, "sensitive_post_parameters", [])
    ...
    def get_post_parameters(self, request: Optional[HttpRequest]) -> Union[QueryDict]:
        ...
        if request is None:
            ...
            sensitive_post_parameters: List[str] = getattr(
                request, "sensitive_post_parameters", []
            )
            if self.is_active(request) and sensitive_post_parameters:
                cleansed: QueryDict = request.POST.copy()
    ...
    def get_traceback_frame_variables(self, request: HttpRequest, tb_frame: types.FrameType) -> Iterator[Tuple[str, Any]]:
        ...
        stars (*********).
        """
        sensitive_variables: Optional[Union[List[str], str]] = None
...
class ExceptionReporter:
    ...
    def get_traceback_data(self):
        ...
        frames: List[Dict[str, Any]] = self.get_traceback_frames()
        for i, frame in enumerate(frames):
            if "vars" in frame:
                frame_vars: List[Tuple[str, str]] = []
    ...
    def _get_lines_from_file(
        ...
        """
        source: Optional[List[Union[str, bytes]]] = self._get_source(filename, loader, module_name)
        if source is None:
            return None, [], None, []
        ...
        upper_bound: int = lineno + context_lines

        try:
            pre_context: str = source[lower_bound:lineno]
            context_line: str = source[lineno]
            post_context: str = source[lineno + 1 : upper_bound]
        except IndexError:
            return None, [], None, []
    ...
    def get_traceback_frames(self) -> List[Dict[str, Any]]:
        ...
        exc_value: BaseException = self.exc_value
        while exc_value:
        ...
        tb: Optional[types.TracebackType] = self.tb if not exceptions else exc_value.__traceback__
        while True:
    ...
    def get_exception_traceback_frames(self, exc_value: BaseException, tb: Optional[types.TracebackType]) -> Iterator[Dict[str, Any]]:
        ...
        while tb is not None:
            ...
            lineno: int = tb.tb_lineno - 1
            loader: Optional[Loader] = tb.tb_frame.f_globals.get("__loader__")
            module_name: str = tb.tb_frame.f_globals.get("__name__") or ""
...
def technical_404_response(request: HttpRequest, exception: Exception) -> HttpResponse:
    ...
    tried: Optional[List[Tuple[ResolverMatch]]]
    try:
    ...
    urlconf: Optional[Union[str, types.ModuleType]] = getattr(request, "urlconf", settings.ROOT_URLCONF)
    if isinstance(urlconf, types.ModuleType):
        urlconf = urlconf.__name__

    with builtin_template_path("technical_404.html").open(encoding="utf-8") as fh:
        t: Template = DEBUG_ENGINE.from_string(fh.read())
    reporter_filter: TypeVar = get_default_exception_reporter_filter()
    c: Context = Context(
        {
            "urlconf": urlconf,
